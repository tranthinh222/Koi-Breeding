...
export default function AuthModal({
	...
	const handleForgotSendCode = async () => {
		try {
			...
			setAuthNotice(
				"Verification code sent to your email. Enter code below. ",
			);
			setForgotStep("code");
			setForgotCode("");
		} catch (error: any) {
				...
				<div className="field-group">
					...
					<label className="input-field date-field">
						...
						<input
							...
							pattern="\\d{2}/\\d{2}/\\d{4}"
							onChange={(e) => {
								const nextBirthday = formatBirthdayInput(
									e.target.value,
								);
								const birthdayApiValue =
									parseBirthdayInput(nextBirthday);

								if (
									birthdayApiValue &&
									birthdayApiValue > maxBirthday
								) {
									setRegisterError(
	...
	const renderForgotView = () => (
		<div className="auth-forgot-layout">
			<section className="auth-panel auth-panel--forgot">
				...
				{forgotError && (
					<div className="auth-error-message">{forgotError}</div>
				)}
