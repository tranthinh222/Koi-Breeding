...
export default function AuthModal({
  ...
  const [, setShowRegisterConfirmPassword] = useState(false);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      if (!USERNAME_REGEX.test(username)) {
        throw new Error(
          "Username can only contain letters, numbers. Please avoid special characters or spaces.",
        );
      }
      if (!EMAIL_REGEX.test(email.trim())) {
        throw new Error("Invalid email format");
      }
      const birthdayApiValue = parseBirthdayInput(birthday);
      if (!birthdayApiValue) {
        throw new Error("Birthday must use the format dd/mm/yyyy");
      }
      if (birthdayApiValue > maxBirthday) {
      ...
      if (!PASSWORD_CHARSET_REGEX.test(password)) {
        throw new Error(
          "Password can only contain ASCII characters without spaces",
        );
      }
      ...
      setRegisterError(getApiErrorMessage(error, "Register failed"));
    } finally {
  ...
  const handleForgotSendCode = async () => {
    try {
      ...
      setForgotError(getApiErrorMessage(error, "Send code failed"));
    } finally {
  ...
  const handleVerifyCode = async () => {
    try {
      ...
      setForgotError(getApiErrorMessage(error, "Verification failed"));
    } finally {
      setForgotLoading(false);
    }
  };
  ...
  const handleResetPassword = async () => {
    try {
      ...
      if (!PASSWORD_CHARSET_REGEX.test(newPassword)) {
        throw new Error(
          "Password can only contain ASCII characters without spaces",
        );
      }
      ...
      setForgotError(getApiErrorMessage(error, "Reset failed"));
    } finally {
      setForgotLoading(false);
    }
  };
        ...
        <div className="field-group">
          ...
          <label className="input-field">
            ...
            <input
              ...
              value={username}
              pattern="[A-Za-z0-9_]+"
              title="Use only English letters, numbers, and underscores"
          ...
          <label className="input-field date-field">
            ...
            <input
              ...
              pattern="\\d{2}/\\d{2}/\\d{4}"
              onChange={(e) => {
                const nextBirthday = formatBirthdayInput(e.target.value);
                const birthdayApiValue = parseBirthdayInput(nextBirthday);

                if (birthdayApiValue && birthdayApiValue > maxBirthday) {
                  setRegisterError("Birthday cannot be later than today");
                  return;
                }
        ...
        <div className="field-group">
          <label className="input-field">
            ...
            <input
              ...
              value={password}
              pattern="[\\x21-\\x7E]+"
              title="Use ASCII characters without spaces"
              onChange={(e) =>
          ...
          <div className="field-group">
            ...
            <label className="input-field">
              ...
              <input
                ...
                title="Use ASCII characters without spaces"
                onChange={(e) =>
                  setConfirmNewPassword(sanitizePasswordInput(e.target.value))
                }
              />
              <button
                type="button"
