...
function BreedingHistory({ onClose }: BreedingHistoryProps) {
			...
			try {
				const response = await callFetchBreedingHistory({
					...
					type:
						filterType === "ALL"
							? undefined
							: (filterType as BreedingType),
					status:
						filterStatus === "ALL"
							? undefined
							: (filterStatus as BreedingStatus),
					pondId:
						filterPond === "ALL" ? undefined : Number(filterPond),
					ended:
						filterIsEnded === "ALL"
							? undefined
							: filterIsEnded === "ENDED",
	...
	const advance = async (eventId: number) => {
		try {
			await callAdvanceBreedingEvent(eventId, currentUserId as number);
			...
			toast.error(message ?? "The next stage is not ready yet.");
		}
	};
				...
				<div className={styles.toolbar}>
					...
					<div className={styles.filterGroup}>
						<select
							...
							value={filterType}
							onChange={(e) =>
								updateFilter(setFilterType, e.target.value)
							}
						>
						...
						<select
							...
							value={filterStatus}
							onChange={(e) =>
								updateFilter(setFilterStatus, e.target.value)
							}
						>
						...
						<select
							...
							value={filterPond}
							onChange={(e) =>
								updateFilter(setFilterPond, e.target.value)
							}
						>
						...
						<select
							...
							value={filterIsEnded}
							onChange={(e) =>
								updateFilter(setFilterIsEnded, e.target.value)
							}
						>
									...
									<div className={styles.parentGroup}>
										<div className={styles.parentKoi}>
											...
											<div className={styles.koiMeta}>
												<span
													className={styles.koiName}
												>
													{event.male.name}
												</span>
												<span className={styles.koiId}>
													(♂ Male)
												</span>
											</div>
										</div>
