...
function PondInformation({ pond, onClose, onEdit }: PondInformationProps) {
  ...
  const resolveAttributeBadge = (attribute: keyof IPond): React.JSX.Element => {
    const BadQuality: React.JSX.Element = (
      <span className={`${styles.qualityBadge} ${styles.badQuality}`}>Bad</span>
    )
    ...
    if (attribute === 'waterQuality') {
      if (pond.waterQuality <= 20) {
        return BadQuality
      }

      if (pond.waterQuality <= 40) {
        return MediumQuality
      }

      if (pond.waterQuality <= 80) {
        return GoodQuality
      }
        ...
        <section className={styles.leftSection}>
          ...
          <div className={styles.textField}>
            ...
            {isEditing ? (
              ...
              <span className={styles.fieldContent}>{pond.name}</span>
            )}
            {isEditing && errors.name && (
              <span className={styles.errorLabel}>{errors.name}</span>
            )}
          </div>
          <div className={styles.textField}>
            <span className={styles.fieldLabel}>Level:</span>
            <span className={styles.fieldContent}>{pond.level}</span>
            <button
              type="button"
          ...
          <div className={styles.textField}>
            ...
            <span className={styles.fieldContent}>{pond.capacity}</span>
          </div>
        ...
        <section className={styles.rightSection}>
          ...
          <div className={styles.statCard}>
            <div className={styles.statInfo}>
              ...
              <div>
                <div className={styles.fieldLabel}>pH Level</div>
                <div className={styles.statValue}>{pond.pH}</div>
              </div>
            </div>
          ...
          <div className={styles.statCard}>
            <div className={styles.statInfo}>
              ...
              <div>
                ...
                <div className={styles.statValue}>{pond.temperature} °C</div>
              </div>
            </div>
          ...
          <div className={styles.statCard}>
            <div className={styles.statInfo}>
              ...
              <div>
                <div className={styles.fieldLabel}>Dissolved Oxygen</div>
                <div className={styles.statValue}>{pond.oxygen} mg/L</div>
              </div>
            </div>
          ...
          <div className={styles.statCard}>
            <div className={styles.statInfo}>
              ...
              <div>
                <div className={styles.fieldLabel}>Water Quality</div>
                <div className={styles.statValue}>{pond.waterQuality}/100</div>
              </div>
            </div>
            {resolveAttributeBadge('waterQuality')}
          </div>

          {/* Card Environment Score (Progress Bar) */}
          <div className={`${styles.statCard} ${styles.progressCard}`}>
            <div className={styles.statInfo}>
              <Gauge size={28} color="#7c3aed" />
              <div className={styles.statValue}>Environment Score</div>
            </div>
            <div className={styles.progressContainer}>
              <div
                className={styles.progressBar}
                style={{ width: `${pond.environmentScore}%` }}
              />
              <span>{pond.environmentScore}/100</span>
            </div>
          </div>
        </section>
      </div>
