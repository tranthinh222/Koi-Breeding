...
export default function AdminAccount() {
	...
	const [form, setForm] = useState({ email: "", birthday: "", gender: "" });
	...
	if (!currentUser)
		return (
			<div className="inline-alert">
				Unable to load the authenticated admin account.
			</div>
		);
	const avatar =
	...
	const save = async (event: FormEvent) => {
		...
		try {
			...
			alert(getApiErrorMessage(error, "Unable to update the profile."));
		} finally {
			setSaving(false);
		}
	};
	...
	const upload = async (blob: Blob) => {
		setUploading(true);
		try {
			const file = new File([blob], `admin-avatar-${Date.now()}.jpg`, {
				type: "image/jpeg",
			});
	...
	return (
		<>
			<div className="settings-page admin-account-view">
				<div className="page-heading">
					<div>
						<p className="eyebrow">Account</p>
						<h1>Profile and identity</h1>
						<p>
							These details come from your authenticated backend
							account.
						</p>
					</div>
					<span className="account-security-badge">
						<ShieldCheck size={16} /> Protected admin
					</span>
				</div>
				<div className="account-layout">
					...
					<form
						className="settings-card account-form-card"
						onSubmit={save}
					>
