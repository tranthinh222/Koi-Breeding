...
import BreedingRecipeCard from "../../components/breeding/BreedingRecipeCard/BreedingRecipeCard";
import FilterModal, {
  type IFilterState,
...
import type { IBreedingRecipe, IKoi, IPond } from "../../types/backend";
...
function Breeding() {
  ...
  const [possibleResults, setPossibleResults] = useState<IBreedingRecipe[]>([]);
  ...
  useEffect(() => {
    if (!slot1?.dictionary.id || !slot2?.dictionary.id) {
      setPossibleResults([]);
      return;
    }
    ...
    const fetchPossibleResults = async () => {
      try {
        const response = await callFetchPairRates(
          slot1.dictionary.id as number,
          slot2.dictionary.id as number,
        );
        if (isCurrent) {
          setPossibleResults(response.data.data ?? []);
        }
      } catch (error) {
        if (isCurrent) {
          setPossibleResults([]);
          toast.error("Failed to load possible breeding results.");
          console.error("Failed to fetch possible breeding results", error);
        }
      }
    };

    void fetchPossibleResults();
    return () => {
      isCurrent = false;
    };
  }, [slot1, slot2]);
  ...
  const handleFetchUserKoiList = async (): Promise<IKoi[]> => {
    try {
      ...
      const activeEventsResponse = await callFetchBreedingHistory({
        userId: currentUserId as number,
        page: 0,
        size: 100,
        ended: false,
      });
      const breedingKoiIds = new Set(
        (activeEventsResponse.data.data?.result ?? []).flatMap((event) => [
          event.male.id,
          event.female.id,
        ]),
      );
      return responses
        ...
        .filter((koi) => !breedingKoiIds.has(koi.id));
    } catch (error) {
  ...
  const handleStartBreeding = async (targetPond: IPond) => {
    ...
    try {
      ...
      setKoiList((list) =>
        list.filter((koi) => koi.id !== slot1.id && koi.id !== slot2.id),
      );
    } catch (error) {
      const message = (error as { response?: { data?: { message?: string } } })
        .response?.data?.message;
      toast.error(
        message ?? "Could not start breeding. Please check the selected pond.",
      );
    } finally {
  ...
  const handleBreedClick = () => {
    ...
    if (slot1.gender === slot2.gender) {
      toast.warning("Breeding usually requires one Male and one Female Koi!");
      return;
    }
  ...
  const applyFilter = (list: IKoi[], filter: IFilterState) => {
    return list.filter((koi) => {
      ...
      const matchPond =
        filter.pondId === "ALL" || koi.pondId.toString() === filter.pondId;
      const matchVariety =
        filter.variety === "ALL" || koi.dictionary.name === filter.variety;
    ...
    <>
      <div className={styles.mainContent}>
        ...
        <div className={styles.contentSection}>
          {/* MALE PARENT */}
          <div className={`${styles.parentsSection} ${styles.maleSection}`}>
            ...
            <div className={styles.parentList}>
              {filteredList1.length === 0 && (
                <p className={styles.emptyParentList}>No male koi found</p>
              )}
          ...
          <div className={styles.breedingSection}>
            ...
            <div className={styles.mainBreedingArea}>
              <div className={styles.breedingSlotGroup}>
                <span className={`${styles.slotRole} ${styles.maleRole}`}>
                  <Mars /> Father
                </span>
                <div
                ...
                >
                  {slot1 && (
                    <img
                      src={
                        slot1.dictionary.imageUrl ?? "/kois/koi-fish-null.svg"
                      }
                      alt="Father"
              ...
              >
                ...
                <span className={styles.heartText}>
                  {isReadyToBreed ? "Tap to\nBreed" : "Select 2\nKois"}
                </span>
              </button>

              <div className={styles.breedingSlotGroup}>
                <span className={`${styles.slotRole} ${styles.femaleRole}`}>
                  <Venus /> Mother
                </span>
                <div
                ...
                >
                  {slot2 && (
                    <img
                      src={
                        slot2.dictionary.imageUrl ?? "/kois/koi-fish-null.svg"
                      }
                      alt="Mother"
                    />
                  )}
              ...
              <div className={styles.possibleResultsContainer}>
                ...
                <div className={styles.resultsFrame}>
                  {possibleResults.length > 0 ? (
                    possibleResults.map((recipe) => (
                      <BreedingRecipeCard key={recipe.id} recipe={recipe} />
                    ))
                  ) : (
                    <span className={styles.noResults}>
                      No recorded result for this pair
                    </span>
                  )}
                </div>
              </div>
          ...
          {/* FEMALE PARENT */}
          <div className={`${styles.parentsSection} ${styles.femaleSection}`}>
            <div className={styles.parentHeading}>
              <Venus />
              <div>
                <h2>Mother</h2>
            ...
            <div className={styles.parentList}>
              {filteredList2.length === 0 && (
                <p className={styles.emptyParentList}>No female koi found</p>
              )}
        ...
        <div className={styles.footerSection}>
          <div className={styles.footerLeft}>
            ...
            >
              <img src="/breeding/guide-book.png" alt="breeding guide" />
            </button>
            <button
              type="button"
              className={styles.actionBtn}
            ...
            >
              <img src="/breeding/history.png" alt="breeding history" />
            </button>
          </div>
      ...
      {isGuideOpen && <BreedingGuide onClose={() => setIsGuideOpen(false)} />}
      {isHistoryOpen && (
