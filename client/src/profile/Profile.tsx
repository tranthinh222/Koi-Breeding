    ...
    const fetchProfile = async () => {
      try {
        ...
        const profileResponse = await fetch(
          `/api/v1/users/profile?id=${matchedUser.id}`,
        );
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Some errors occured while loading profile.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadResponse = await fetch(
        `/api/v1/users/avatar?id=${profile.id}`,
        {
          method: "POST",
          body: form,
        },
      ...
      setProfile((current) =>
        current ? { ...current, avatarUrl: uploadData.avatarUrl } : current,
      );
    ...
    <>
      <main className="profile-page">
        <section className="profile-card">
          ...
          >
            {displayedAvatarUrl ? (
              <img src={displayedAvatarUrl} alt={displayName} />
            ) : (
              avatarText
            )}
          </div>
          ...
          <p>
            Level{" "}
            {profile?.exp ? Math.max(1, Math.floor(profile.exp / 100)) : 18}
          </p>
          <button
              ...
              <div className="info-box">
                ...
                <p>
                  <label>
                    ...
                    {editing == false ? (
                      <span>
                        {formatISODate(profile.birthday) || "Not updated."}
                      </span>
                    ) : (
                      <input
                        type="date"
