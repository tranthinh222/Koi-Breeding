...
public class PondFormula {
    ...
    public static int getPHScore(BigDecimal value) {
        ...
        if (pH < 6 || pH > 9) return 0;
        if (pH < 6.5 || pH > 8.5) return 20;
        if (pH < 6.8) return 50;
        if (pH > 8) return 60;
        if (pH < 7) return 80;
        if (pH > 7.5) return 90;
    ...
    public static int getTemperatureScore(BigDecimal value) {
        ...
        if (temperature < 10 || temperature > 32) return 0;
        if (temperature < 15) return 20;
        if (temperature < 20) return 50;
        if (temperature < 22) return 80;
        if (temperature <= 26) return 100;
        if (temperature <= 28) return 90;
    ...
    public static int getWaterScore(BigDecimal waterQuality) {
        ...
        if (waterQuality.compareTo(BigDecimal.valueOf(20)) <= 0) return 20;
        if (waterQuality.compareTo(BigDecimal.valueOf(40)) <= 0) return 40;
        if (waterQuality.compareTo(BigDecimal.valueOf(60)) <= 0) return 60;
        if (waterQuality.compareTo(BigDecimal.valueOf(80)) <= 0) return 80;
        return 100;
    }
    ...
    public static int getOxygenScore(BigDecimal value) {
        ...
        if (oxygen < 3) return 0;
        if (oxygen < 4) return 20;
        if (oxygen < 5) return 50;
        if (oxygen < 6) return 80;
        if (oxygen <= 9) return 100;
        if (oxygen <= 10) return 90;
        if (oxygen <= 12) return 70;
        return 50;
    }
